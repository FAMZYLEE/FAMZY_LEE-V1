...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./start/lib/database/premium.json", JSON.stringify(_dir));
};
...
const expiredPremiumCheck = (client, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./start/lib/database/premium.json", JSON.stringify(_dir));
			idny ? XeonBotInc.sendMessage(idny, { text: "ʏᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ʜᴀꜱ ʀᴜɴ ᴏᴜᴛ 😬, ᴘʟᴇᴀꜱᴇ ᴘᴜʀᴄʜᴀꜱᴇ ᴀɢᴀɪɴ." }) : "";
			idny = false;
		}
	}, 1000);
};
